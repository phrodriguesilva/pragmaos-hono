...
hearingsRoutes.get("/", async (c) => {
  ...
  const rows = (hearings ?? []).map((h) => [
    ...
    <div class="flex items-center gap-2">
      ...
      <form method="post" action={`/hearings/${h.id}/delete`} class="inline" onsubmit="return confirm('Excluir este registro?')"><button type="submit" class="text-status-red hover:underline text-body-sm" aria-label="Excluir">Excluir</button></form>
