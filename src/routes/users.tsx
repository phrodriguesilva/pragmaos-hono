...
usersRoutes.get("/", async (c) => {
  ...
  return renderPage(
    ...
    <>
      <PageHeader title="Usuarios" actions={() => <a href="/users/new" class="btn btn-primary">Novo Usuario</a>} />
      <Table
      ...
      <Panel>
        <form method="post" action="/users" class="flex flex-col gap-4">
          ...
          <div class="flex gap-2">
            <button type="submit" class="btn btn-primary">Criar e Convidar</button>
  ...
  if (error || !data.user) {
    return renderPage(
      ...
      <>
        <PageHeader title="Novo Usuario" />
        <Panel>
          <div class="mb-4 text-status-red">Erro ao convidar: {error?.message ?? "erro desconhecido"}</div>
          <a href="/users/new" class="btn btn-secondary">Voltar</a>
        </Panel>
      </>,
    );
  }
