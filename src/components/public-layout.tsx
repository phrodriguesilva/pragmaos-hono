  ...
  return (
    <html lang="pt-BR">
      <head>
        ...
        {/* Twitter */}
        <meta name="twitter:card" content={tenant.logo_url ? "summary_large_image" : "summary"} />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                <div class="flex items-center gap-2.5 mb-3">
                  ...
                  <span class="text-base font-semibold text-[#0568ff]">{tenant.name}</span>
                </div>
                {tenant.tagline && <p class="text-sm text-[#7a82a0]">{tenant.tagline}</p>}
                {tenant.oab_number && <p class="text-xs text-[#7a82a0] mt-2">OAB: {tenant.oab_number}</p>}
              </div>
                ...
                <ul class="space-y-2 text-sm">
                  {tenant.email_public && (
                    <li class="flex items-center gap-2">
                      <i class="ph ph-envelope text-[#7a82a0]" aria-hidden="true" />
                      <a href={`mailto:${tenant.email_public}`} class="hover:text-[#0568ff]">{tenant.email_public}</a>
                    </li>
                  )}
                  {tenant.phone && (
                    <li class="flex items-center gap-2">
                      <i class="ph ph-phone text-[#7a82a0]" aria-hidden="true" />
                      <a href={`tel:${tenant.phone}`} class="hover:text-[#0568ff]">{tenant.phone}</a>
                    </li>
                  )}
                  {tenant.address && (
                    <li class="flex items-start gap-2">
                      <i class="ph ph-map-pin text-[#7a82a0] mt-0.5" aria-hidden="true" />
              ...
              {/* Social + Links */}
              <div>
                <h4 class="text-sm font-semibold text-[#0568ff] mb-3">Navegação</h4>
                <ul class="space-y-2 text-sm">
                  <li><a href={`${b}/`} class="hover:text-[#0568ff]">Início</a></li>
                  <li><a href={`${b}/areas`} class="hover:text-[#0568ff]">Áreas de Atuação</a></li>
                  <li><a href={`${b}/equipe`} class="hover:text-[#0568ff]">Equipe</a></li>
                  <li><a href={`${b}/artigos`} class="hover:text-[#0568ff]">Artigos</a></li>
                  <li><a href={`${b}/reconhecimentos`} class="hover:text-[#0568ff]">Reconhecimentos</a></li>
                  <li><a href={`${b}/sobre`} class="hover:text-[#0568ff]">Sobre</a></li>
                  <li><a href={`${b}/contato`} class="hover:text-[#0568ff]">Contato</a></li>
                </ul>
                {(tenant.social_facebook || tenant.social_instagram || tenant.social_linkedin) && (
                  <div class="flex gap-3 mt-4">
                    {tenant.social_facebook && <a href={tenant.social_facebook} class="text-[#7a82a0] hover:text-[#0568ff]" aria-label="Facebook"><i class="ph ph-facebook-logo text-lg" aria-hidden="true" /></a>}
                    {tenant.social_instagram && <a href={tenant.social_instagram} class="text-[#7a82a0] hover:text-[#0568ff]" aria-label="Instagram"><i class="ph ph-instagram-logo text-lg" aria-hidden="true" /></a>}
                    {tenant.social_linkedin && <a href={tenant.social_linkedin} class="text-[#7a82a0] hover:text-[#0568ff]" aria-label="LinkedIn"><i class="ph ph-linkedin-logo text-lg" aria-hidden="true" /></a>}
                  </div>
                )}
              </div>
            </div>

            <div class="border-t border-[#dce0e8] mt-8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#7a82a0]">
              <span>&copy; {new Date().getFullYear()} {tenant.name}. Todos os direitos reservados.</span>
              <div class="flex gap-4">
                <a href={`${b}/lgpd`} class="hover:text-[#0568ff] transition">Política de Privacidade</a>
                <a href={`${b}/lgpd/termos`} class="hover:text-[#0568ff] transition">Termos de Uso</a>
              </div>
